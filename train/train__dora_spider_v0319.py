# train__dora_spider_v0319.py
# 使用 DoRA (Weight-Decomposed LoRA) 微調 Llama-3.1-8B-Instruct
#
# 改動重點（vs 0318）：
#   - 訓練集換為新英文資料集 train_claude_en_2000.json（1775 筆）
#   - 驗證集換為 val_claude_en_spider.json（242 筆）
#   - TABLE_NOTES 同步強化版（no-isDel views 明確禁止）
#
# 執行方式:
#   python train__dora_spider_v0319.py
#
# 輸出:
#   outputs/models/wp_m09_dora_0319_spider/final_model/
#
# 評估:
...
TRAIN_PATH  = r"data\wp_m09\train_claude_en_2000.json"   # 1775 筆英文
VAL_PATH    = r"data\wp_m09\val_claude_en_spider.json"   # 242 筆英文驗證
DATE_STR    = "0319"
...
def save_model(trainer, tokenizer, train_samples: int):
    ...
    info = {
        ...
        "train_format":    "Spider 1.0 (English)",
        ...
        "warmup_ratio":    WARMUP_RATIO,
        "date":            DATE_STR,
        "changes":         "English training set (1775 samples generated by Claude)",
...
def main():
    print("=" * 60)
    print(f"WP_M09 DoRA 訓練 v{DATE_STR}（英文訓練集）")
    ...
    print(f"   評估指令: python eval__en_spider_val.py --model {FINAL_MODEL}")
