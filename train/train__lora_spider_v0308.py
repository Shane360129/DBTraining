# train_lora_spider0308.py
# 使用純 LoRA 微調 Llama-3.1-8B-Instruct
# 訓練資料: data/wp_m09/train_spider_format_fixed.json  (Spider 1 格式)
#
# 執行方式:
#   python train_lora_spider0308.py
#
# 輸出:
#   outputs/models/wp_m09_lora_{MMDD}_spider/final_model/
...
TRAIN_PATH   = r"data\wp_m09\train_spider_format_fixed.json"
DATE_STR     = datetime.now().strftime("%m%d")
OUTPUT_DIR   = f"outputs/models/wp_m09_lora_{DATE_STR}_spider"
...
LORA_ALPHA    = 32
LORA_DROPOUT  = 0.05
USE_DORA      = False     # ← 純 LoRA

# ---- 訓練超參數 ----
NUM_EPOCHS    = 15
BATCH_SIZE    = 4
GRAD_ACCUM    = 4         # effective batch = 16
LEARNING_RATE = 2e-4      # LoRA 通常用較大 LR
MAX_SEQ_LEN   = 512
LR_SCHEDULER  = "cosine"
WARMUP_RATIO  = 0.08
WEIGHT_DECAY  = 0.01
...
def load_dataset(path: str) -> Dataset:
    ...
    diff_cnt = Counter(s.get("difficulty", "unknown") for s in raw)
...
def apply_lora(model):
    lora_cfg = LoraConfig(
        ...
        use_dora=USE_DORA,   # False → 純 LoRA
    )

    model = get_peft_model(model, lora_cfg)
    model.print_trainable_parameters()
    print(f"微調方法: LoRA  (r={LORA_R}, alpha={LORA_ALPHA})")
    return model
...
def save_model(trainer, tokenizer):
    ...
    info = {
        ...
        "train_script":    "train_lora_spider0308.py",
        "method":          "LoRA",
        "lora_r":          LORA_R,
        "lora_alpha":      LORA_ALPHA,
        "epochs":          NUM_EPOCHS,
...
def main():
    print("=" * 60)
    print(f"WP_M09 LoRA 訓練 (Spider 格式)  ({DATE_STR})")
    print(f"腳本: train_lora_spider0308.py")
    print("=" * 60)
    ...
    print(f"   (記得更新 MODEL_PATH 為 {FINAL_MODEL})")
