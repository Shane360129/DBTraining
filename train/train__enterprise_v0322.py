# train__enterprise_v0322.py
# ============================================================
# 企業 Text-to-SQL 落地實驗 — 訓練腳本
#
# 方法論：仿照 Spider 1.0 / BIRD Benchmark 訓練方式
#   - Spider: 全資料庫 schema 放入 prompt，模型自行選表
#   - BIRD:   schema + evidence (領域知識提示)
#   - 本腳本: 全 7 View schema + 企業商業邏輯規則
#
# vs v0322 關鍵改動：
#   1. 全 7 View 的 CREATE TABLE 放入每個 prompt（非單表）
#      → 模型學會自行選表，消除推論時的 keyword 推斷失敗
#   2. 統一商業規則區塊（BIRD evidence 概念）
#   3. 內建弱模式資料擴增（OutStock、子查詢去重、isDel 辨別）
#   4. MAX_SEQ_LEN=1280（緊湊 schema ~937 tokens + Q+SQL ~130 tokens）
#   5. BATCH_SIZE=2, GRAD_ACCUM=8（配合較長序列的 VRAM）
#
# 實驗設計（論文結構）：
#   Exp 1: 本腳本訓練 → val_v2 評估 (EX/EM, per-view, per-difficulty)
#   Exp 2: Ablation (w/o schema, w/o rules, w/o augment, single-table vs full)
#   Exp 3: 與 Spider/BIRD 同方法論對比
#
# Token 長度：
#   System prompt (全7表 + rules): ~937 tokens
#   最短 sample:                    ~992 tokens
#   最長 sample:                    ~1,066 tokens (含長 SQL)
#   MAX_SEQ_LEN=1280:               所有 sample 都能容納 (margin ~214)
#
# 硬體需求：RTX 5070 Ti 16GB (4-bit quant, batch=2)
#
# 用法:
#   python train__enterprise_v0322.py
#   python train__enterprise_v0322.py --no-augment     # 不擴增（ablation）
#   python train__enterprise_v0322.py --schema single   # 單表模式（ablation）
#   python train__enterprise_v0322.py --no-rules        # 不含規則（ablation）
#
# 輸出:
#   outputs/models/enterprise_fullschema_0322/final_model/
# ============================================================
...
TRAIN_PATHS  = [
    r"data\wp_m09\train_spider_WP_M09.json",       # 1,014
    r"data\wp_m09\train_claude_en_2000.json",        # 1,748
]
...
FULL_SCHEMA = (
    ...
    " -- Transfer. LEFT(TransferId,8)=date. isDel+dtlIsDel. No header amount.\n"
...
def save_model(trainer, tokenizer, n_samples, output_dir, args):
    ...
    info = {
        ...
        "train_script":     "train__enterprise_v0322.py",
...
def main():
    ...
    print("=" * 70)
    print(f"Enterprise Text-to-SQL Training v{DATE_STR}")
    ...
    dataset, n_samples = load_data(tokenizer, schema_mode=args.schema,
                                    include_rules=not args.no_rules,
    ...
    if args.schema == "full":
        ...
        print(f"python train__enterprise_v0322.py --schema single       # Single-table (v0322 style)")
        print(f"python train__enterprise_v0322.py --schema none         # No schema baseline")
        print(f"python train__enterprise_v0322.py --no-augment          # No augmentation")
        print(f"python train__enterprise_v0322.py --no-rules            # No business rules")
