# train__enterprise_v0323.py
# ============================================================
# 企業 Text-to-SQL 落地實驗 — 訓練腳本 v0323
#
# 方法論：仿照 Spider 1.0 / BIRD Benchmark
#
# vs v0322 關鍵改動：
#   1. 修正訓練資料：SUM(DISTINCT)→子查詢去重(220筆), pvSn→pvId(189筆), COUNT(*)→COUNT(DISTINCT)(33筆)
#   2. 新增 201 筆針對性擴增（欄位選擇、TOP N、子查詢、SELECT *、中文）
#   3. 強化 BUSINESS_RULES（8 條→含負面規則 NEVER SUM(DISTINCT)/LIMIT）
#   4. Schema 增加欄位用途提示（Provider: use pvId not pvSn）
#   5. Epochs 6→8（修正後資料需更多迭代覆蓋）
#
# 實驗設計（論文結構）：
...
#
# 用法:
#   python train__enterprise_v0323.py
#   python train__enterprise_v0323.py --no-augment     # 不擴增（ablation）
#   python train__enterprise_v0323.py --schema single   # 單表模式（ablation）
#   python train__enterprise_v0323.py --no-rules        # 不含規則（ablation）
#
# 輸出:
...
def save_model(trainer, tokenizer, n_samples, output_dir, args):
    ...
    info = {
        ...
        "train_script":     "train__enterprise_v0323.py",
...
def main():
    ...
    if args.schema == "full":
        ...
        print(f"python train__enterprise_v0323.py --schema single       # Single-table (v0322 style)")
        print(f"python train__enterprise_v0323.py --schema none         # No schema baseline")
        print(f"python train__enterprise_v0323.py --no-augment          # No augmentation")
        print(f"python train__enterprise_v0323.py --no-rules            # No business rules")
